...
export class ShowErrorsComponent implements OnInit {
  ...
  ERROR_MESSAGE = {
    ...
    minlength: (par) => `${this.label} must contain at least ${par.requiredLength} characters.`,
    maxlength: (par) => `${this.label} may contain at a maximum of ${par.requiredLength} characters.`,
    min: (par) => `${this.label} must be at least ${par.min}.`,
    max: (par) => `${this.label} may not exceed ${par.max}.`,
    pattern: (par) => `Valid pattern ${par.requiredPattern}`,
